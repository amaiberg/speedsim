...
int IGTime; /* estimated length of initial growth interval (after delay) */
int IGRDataLen;
int peakIdx;
int IGRDataCnt; /* current # entries in IGRData */
int recordIGRdata; /* whether to continue keeping track of I vals */
double *IGRData;
double *IGRtData;
...
/* for Hilbert stuff at end */
double **htemp;

/* proportions for drawing */
double **drawable;
double minS=65536, maxS=0.0;

double MU,  PHI,  GAMMA,  MINTIME, MAXTIME,  TIMEINTERVAL,  PLONG_A,  PMED_A, PSHORT_A,  PLONG_B,  PMED_B,  PSHORT_B,  PSWITCH;
char *landFname, *logFname;
SimType SIMTYPE; 
unsigned int randomSeed; 
FILE * LandscapePointer;
long off,  len;
int ONELINE,  seeded,rank, NUMI0 ;

void init(int size, int aNUMI0, double aMU, double aPHI, double aGAMMA, double aMINTIME,
		double aMAXTIME, double aTIMEINTERVAL, double aPLONG_A, double aPMED_A,
		double aPSHORT_A, double aPLONG_B, double aPMED_B, double aPSHORT_B, double aPSWITCH,
		char *alandFname, char *alogFname, SimType aSIMTYPE, int aONELINE, int aseeded,
		unsigned int arandomSeed, int arank, FILE * aLandscapePointer, long aoff, long alen) {
	...
	PSHORT_B=aPSHORT_B;
	PSWITCH=aPSWITCH;
	landFname =alandFname; 
	logFname=alogFname;
	SIMTYPE=aSIMTYPE; 
	randomSeed=arandomSeed; 
	LandscapePointer = aLandscapePointer;
	off=aoff; 
	len=alen;
	ONELINE=aONELINE;
	seeded = aseeded; 
	...
	htemp = (double**) malloc(NUM_N*sizeof(double*));
	Shh = (double**) malloc(NUM_N*sizeof(double*));
	Rhh = (double**) malloc(NUM_N*sizeof(double*));

	drawable = (double**) malloc(NUM_N*sizeof(double*));

	fseek(LandscapePointer, off, SEEK_SET);
	...
	for (i = 0; i < NUM_N; i++) {
		...
		htemp[i] = (double*) malloc(HH_PER_N*sizeof(double));
		Rhh[i] = (double*) malloc(HH_PER_N*sizeof(double));
		Shh[i] = (double*) malloc(HH_PER_N*sizeof(double));

		drawable[i] = (double*) malloc(HH_PER_N*sizeof(double));
	...
	for (i = 0; i < NUM_N; i++) {
		for (j = 0; j < HH_PER_N; j++) {
			drawable[i][j] = (Shh[i][j]-minS+1.0)/(maxS-minS)*100.0;
		}
	}
	...
	/* for drawing */
	if(Shh[nind][hind] < minS)
		minS = Shh[nind][hind];
	drawable[nind][hind] = ((Ihh_A[nind][hind]-minS+1)/(maxS-minS)*100.0)+100.0;
...
double ** next(){
	...
	if (eventrnd < (rSI/totalRate)) {
		...
		if (simType == SIS) {
			...
			/* for drawing */
			if(Shh[nind][hind] > maxS)
				maxS = Shh[nind][hind];
			if(Ihh_A[nind][hind] == 0)
				drawable[nind][hind] = (Shh[nind][hind]-minS+1.0)/(maxS-minS)*100.0;
			else
				drawable[nind][hind] = ((Ihh_A[nind][hind]-minS+1)/(maxS-minS)*100.0)+100.0;
			...
			/* for drawing */
			drawable[nind][hind] = ((Ihh_A[nind][hind]-minS+1.0)/(maxS-minS)*100.0)+100.0;
		}
	...
	return hilbert_embed(drawable, htemp, (int)sqrt((double)SIZE));
}
...
void doInfection(double plongSucc, double pmedSucc, double pSuccess, double *Sn,
	...
	if(Shh[nind][hind] < minS)
		minS = Shh[nind][hind];
	drawable[nind][hind] = ((Ihh_A[nind][hind]-0.0+1.0)/(694.0-0.0)*694.0)+100.0;
}
